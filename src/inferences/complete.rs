...
fn contradiction() -> Inferred {
    set![Goal::new(set![Arc::new(F)])]
}

fn equality(goal: &Goal, left: &Arc<Term>, right: &Arc<Term>) -> Inferred {
    let smaller = min(left, right);
    let larger = max(left, right);
    let mut replaced: Set<_> = goal.formulae()
        .map(|f| f.replace(smaller, larger))
        .collect();
    replaced.insert(Arc::new(Eql(left.clone(), right.clone())));
    set![Goal::new(replaced)]
}

fn negated(goal: &Goal, p: &Arc<Formula>) -> Set<Inferred> {
    if goal.contains(p) {
    ...
    match **p {
        ...
        Imp(ref p, ref q) => set![set![
            goal.with(p.clone()).with(q.negated())
        ]],
        Eqv(ref p, ref q) => {
        ...
        And(ref ps) => {
            let nps = Arc::new(Or(ps.iter().map(|x| x.negated()).collect()));
            set![set![goal.with(nps)]]
        }
        Or(ref ps) => {
            let nps = Arc::new(And(ps.iter().map(|x| x.negated()).collect()));
            set![set![goal.with(nps)]]
        },
        ...
        All(ref p) => set![set![
            goal.with(p.instantiate_with_constant().negated())
        ]]
    }
}
...
fn formula_inferences(goal: &Goal, f: &Arc<Formula>) -> Set<Inferred> {
    match **f {
        ...
        Or(ref ps) => ps.iter()
            .map(|p| set![goal.with(p.clone())])
            .collect(),
        Ex(ref p) => set![set![
            goal.with(p.instantiate_with_constant())
        ]],
        All(ref p) => goal.symbols()
            .map(|s| set![
                 goal.with(p.instantiate_with_symbol(*s))
            ])
            .collect::<Set<_>>()
...
pub fn inferences(goal: &Goal) -> Set<Inferred> {
    Set::unions(goal.formulae()
        .map(|f| formula_inferences(goal, f))
    )
}
