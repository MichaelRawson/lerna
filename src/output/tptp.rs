...
fn to_tptp_term(t: Uniq<Term>, bound_depth: usize) -> Box<syntax::FofTerm> {
    Box::new(match *t {
...
fn to_tptp_formula(f: Uniq<Formula>, bound_depth: usize) -> Box<syntax::FofFormula> {
    Box::new(match *f {
        T => Boolean(true),
        F => Boolean(false),
        Eql(left, right) => Infix(
...
fn to_tptp_statement(index: usize, f: Uniq<Formula>) -> syntax::Statement {
...
fn print_refutation(start: &Set<Uniq<Formula>>, proof: Proof, mut index: usize) -> usize {
...
pub fn szs_refutation(name: &str, start: &Set<Uniq<Formula>>, proof: Proof) {
